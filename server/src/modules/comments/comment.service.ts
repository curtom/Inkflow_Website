...
export async function deleteComment(userId: string, slug: string, commentId: string) {
  ...
  const deletedCommentId = comment._id as mongoose.Types.ObjectId;
  await Comment.updateMany(
    { article: article._id, parentComment: deletedCommentId },
    { $set: { parentComment: null } }
  );
  await Comment.deleteOne({ _id: deletedCommentId });

  article.commentsCount = Math.max(0, (article.commentsCount ?? 0) - 1);
  if (article.pinnedComment && String(deletedCommentId) === String(article.pinnedComment)) {
