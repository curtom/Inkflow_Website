...
export default function NotificationsPage() {
  ...
  if (isLoading) {
    return (
      <div className="mx-auto max-w-6xl px-4 py-10">
        <p className="text-stone">加载通知中…</p>
      </div>
    );
  }
  ...
  if (isError || !data?.data) {
    return (
      <div className="mx-auto max-w-6xl px-4 py-10">
        <p className="text-error">无法加载通知。</p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      <h1 className="mb-6 text-3xl font-bold text-ink">消息通知</h1>

      <div className="mb-6 flex gap-2 border-b border-border-cream">
        <button
          type="button"
      ...
      {tab === "interaction" ? (
                ...
                if (item.kind === "comment") {
                  return (
                    <Link
                      ...
                      className="flex gap-3 rounded-xl border border-border-cream p-3 transition hover:border-terracotta/35 hover:bg-parchment"
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-sm text-ink">
                          ...
                          <span className="text-charcoal"> 评论了你的文章</span>
                          <span className="font-medium text-terracotta"> 《{item.article.title}》</span>
                        </p>
                        <p className="mt-1 line-clamp-2 text-xs text-charcoal">{item.excerpt}</p>
                        <p className="mt-1 text-xs text-stone">{formatTime(item.createdAt)}</p>
                      </div>
                    </Link>
                  );
                }
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-sm text-ink">
                          <span className="font-semibold">{item.actor.username}</span>
                          <span className="text-charcoal"> 发布了新文章</span>
                          <span className="font-medium text-terracotta"> 《{item.article.title}》</span>
                        </p>
                        <p className="mt-1 text-xs text-stone">{formatTime(item.createdAt)}</p>
                      </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-sm text-ink">
                        <span className="font-semibold">{item.actor.username}</span>
                        <span className="text-charcoal"> {verb}</span>
                        <span className="font-medium text-terracotta"> 《{item.article.title}》</span>
                      </p>
                      <p className="mt-1 text-xs text-stone">{formatTime(item.createdAt)}</p>
                    </div>
        ...
        <div className="space-y-8">
          <section className="rounded-2xl border border-border-cream bg-ivory p-5 shadow-whisper">
            <h2 className="mb-4 text-lg font-bold text-ink">粉丝动态</h2>
            <div className="space-y-3">
              {followNotifications.length === 0 ? (
                <p className="text-sm text-stone">暂无新的关注记录。</p>
              ) : (
                followNotifications.map((item) => (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-sm text-ink">
                        <span className="font-semibold">{item.actor.username}</span>
                        <span className="text-charcoal"> 关注了你</span>
                      </p>
                      <p className="mt-1 text-xs text-stone">{formatTime(item.createdAt)}</p>
                    </div>
                  </Link>
          ...
          <section className="rounded-2xl border border-border-cream bg-ivory p-5 shadow-whisper">
            <h2 className="mb-4 text-lg font-bold text-ink">关注你的人（{followers.length}）</h2>
            <div className="max-h-[60vh] space-y-3 overflow-y-auto pr-1">
              {followers.length === 0 ? (
                <p className="text-sm text-stone">还没有人关注你。</p>
              ) : (
                followers.map((item) => (
                  <Link
                    key={item.user.id}
                    to={`/profiles/${encodeURIComponent(item.user.username)}`}
                    className="flex items-center gap-3 rounded-xl border border-border-cream p-3 transition hover:border-terracotta/35 hover:bg-parchment"
                  >
                    <UserAvatar username={item.user.username} avatar={item.user.avatar} />
                    <div className="min-w-0">
                      <p className="font-medium text-ink">{item.user.username}</p>
                      <p className="truncate text-xs text-stone">{formatTime(item.followedAt)}</p>
                    </div>
                  </Link>
                ))
              )}
